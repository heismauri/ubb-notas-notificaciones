...
beforeAll(async () => {
  await env.DATA.put("students", JSON.stringify([{ run, firstName: "Test", lastName: "User" }]));
});
...
describe("Refresh courses", () => {
  ...
  it("responds with a success message", async () => {
    fetchMock
      .get(env.BASE_URL)
      .intercept({ path: `/v2/config/get_carreras/${run}` })
      .reply(200, carreras);
    fetchMock
      .get(env.BASE_URL)
      .intercept({ path: `/calificaciones/get_asignaturas/${run}/29370/2/2025/1/2025/2` })
    ...
    fetchMock
      .get(env.BASE_URL)
      .intercept({ path: `/calificaciones/get_calificaciones/${run}/6205031/2/2025/2` })
      .reply(200, aCalificaciones);
    fetchMock
      .get(env.BASE_URL)
      .intercept({ path: `/calificaciones/get_calificaciones_modular/${run}/2201561/1/2025/2/29370/2/1/1` })
      .reply(200, mICalificaciones);
    fetchMock
      .get(env.BASE_URL)
      .intercept({ path: `/calificaciones/get_calificaciones_modular/${run}/2201561/1/2025/2/29370/2/2/1` })
